...
class EventProcessor (
    ...
    override fun finish() {
        ...
        val code = buildString {
            ...
            appendLine("                val param = params[0]")
            appendLine("                if (params.size == 1 && org.bukkit.event.Event::class.java.isAssignableFrom(params[0])) {")
            appendLine("                    val eventClass = param.asSubclass(org.bukkit.event.Event::class.java)")
            ...
            appendLine("                                plugin.logger.severe(\"[EventRegistry] Error executing \${clazz.name}#\${method.name}: \${ex.message}\")")
            appendLine("                                ex.printStackTrace()")
            appendLine("                            }")
            appendLine("                        }")
            appendLine("                    }, plugin, ann.ignoreCancelled)")
            appendLine("                } else {")
            appendLine("                    plugin.logger.warning(\"[EventRegistry] Method \${clazz.name}#\${method.name} has @Event annotation but invalid parameters.\")")
            appendLine("                }")
            appendLine("            }")
            appendLine("        }")
            appendLine("    }")
